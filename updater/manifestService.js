...
function validateManifest(manifest, allowedNamespace = "ghcr.io/joxdev12") {
  ...
  for (const key of ["backend", "frontend", "updater"]) {
    ...
    const escapedNamespace = allowedNamespace.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const referenceOk = IMAGE_RE.test(image?.reference || "") || new RegExp(`^${escapedNamespace}/ricercacasa-(backend|frontend|updater)@sha256:[0-9a-f]{64}$`).test(image?.reference || "");
    if (!image || !referenceOk || !VERSION_RE.test(image.version || "")) throw new Error(`Manifest immagine ${key} non valida`);
  }
